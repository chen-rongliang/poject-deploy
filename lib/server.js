...
SentServer.fn = SentServer.prototype = {
    init: function (options) {
        ...
        this.data.to = (!options.to || options.to === '/') ? '' : options.to
            ...
            res
                ...
                .on('end', function () {
                    if (status >= 200 && status < 300 || status === 304) {
                        // 发送成功时打印一条信息
                        console.log(`[${_self.getTime()}]:${_self.files[idx].path} >> ${_self.data.to + _self.files[idx].to}.`)
                    } else {
                        // 显示服务器返回信息
                        console.log(`[${_self.getTime()}]:upload file:[${_self.files[idx].name}] failed[code:${status}].`)
                    }
                    // 销毁数据
                    collect = opt = null
                    // 执行回调
